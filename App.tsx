...
export default function App() {
  ...
  function startLesson() {
    ...
    // Five common words are presented through three different strategies so their outcomes can be compared.
  ...
  async function finishLesson() {
    ...
    const nextData: LearningData = {
      ...
      retention: [
        ...data.retention,
        createRetentionRecord(session.id, session.createdAt, 24),
        createRetentionRecord(session.id, session.createdAt, 168),
      ],
    };
  ...
  async function answerRetention(value: string) {
    if (!current || retentionAnswers.length > retentionIndex) return;
    const correct = value === current.word;
    setRetentionAnswers((prev) => [...prev, { wordId: current.id, strategy: current.strategy, correct, responseMs: 0, attempts: 1, hints: 0 }]);
  }

  async function finishRetention() {
    if (!retentionRecordId) return;
    const updated = data.retention.map((record) => record.sessionId === retentionRecordId ? { ...record, completedAt: Date.now(), answers: retentionAnswers } : record);
    // retentionRecordId stores sessionId; one session can have both 24h and 7d records.
  ...
  function retentionWords(record: { sessionId: string }) {
    ...
    if (!session) return [];
    const strategyByWord: Record<string, Strategy> = {};
    session.answers.forEach((a) => { strategyByWord[a.wordId] = a.strategy; });
    return session.answers.map((a) => {
      const word = WORDS.find((w) => w.id === a.wordId);
      return word ? { ...word, strategy: strategyByWord[a.wordId] } : null;
  ...
  if (screen === 'home') {
    ...
    return <SafeAreaView style={styles.safe}><StatusBar style="dark" /><ScrollView contentContainerStyle={styles.container}>
      ...
      <Text style={styles.sectionTitle}>El experimento</Text>
      {STRATEGIES.map((s, i) => <View style={styles.strategy} key={s.key}><View style={styles.number}><Text style={styles.numberText}>{i + 1}</Text></View><View style={styles.strategyBody}><Text style={styles.strategyTitle}>{s.icon} {s.title}</Text><Text style={styles.strategyText}>{s.subtitle}</Text></View></View>)}
      <TouchableOpacity style={styles.primary} onPress={startLesson}><Text style={styles.primaryText}>{completed ? 'Entrenar de nuevo' : 'Comenzar experimento'}</Text></TouchableOpacity>
      <Text style={styles.footer}>La personalización de LÜ se construye con datos de rendimiento, no con etiquetas.</Text>
  ...
  if (screen === 'lesson' && current) {
    ...
    return <SafeAreaView style={styles.safe}><StatusBar style="dark" /><View style={styles.lessonContainer}>
      ...
      {isContext && <Text style={styles.question}>¿Qué palabra completa la situación?</Text>}
      {isAudio && <Text style={styles.question}>¿Qué palabra escuchaste?</Text>}
      ...
      {hints > 0 && !selected && <Text style={styles.hintDetail}>Pista: la palabra comienza con “{current.word[0]}”.</Text>}
  ...
  if (screen === 'result') {
    ...
    const score = last?.answers.filter((a) => a.correct).length ?? 0;
  ...
  if (screen === 'retention') {
    const record = data.retention.find((r) => r.sessionId === retentionRecordId && r.completedAt === null && r.dueAt <= Date.now());
    const words = record ? retentionWords(record) : [];
